/*
  ...
  All policies that check admin/field_manager/weighbridge roles by querying
  user_profiles table cause infinite recursion when applied to other tables
  that themselves have policies requiring user_profiles lookups.

  Fix: Replace all user_profiles subquery checks with a helper function
  that uses security definer to bypass RLS, breaking the recursion chain.
*/
