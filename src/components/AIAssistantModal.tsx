    ...
    <>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="flex border-b border-slate-100 bg-slate-50/70 px-3 pt-2 gap-1 text-xs font-semibold overflow-x-auto no-scrollbar max-w-full">
            <button
          ...
          {/* Content Area */}
          <div className="flex-1 overflow-hidden flex flex-col bg-slate-50/50 relative min-w-0 max-w-full">
            {/* TAB 1: CHAT */}
            {activeTab === 'chat' && (
              <>
                ...
                {/* Quick Prompts Bar */}
                <div className="overflow-x-auto py-2 px-3 border-b border-slate-100 bg-white flex gap-1.5 no-scrollbar shrink-0 items-center max-w-full">
                ...
                <div className="flex-1 overflow-y-auto overflow-x-hidden p-4 space-y-3.5 min-w-0 max-w-full">
                  {messages.map((msg) => (
                    <div
                      key={msg.id}
                    ...
                    >
                      <div
                        className={`group relative max-w-[90%] break-words rounded-2xl px-3.5 py-2.5 shadow-sm text-sm ${
                          msg.role === 'user'
                ...
                {/* Input Bar */}
                <div className="p-2.5 sm:p-3 pb-16 sm:pb-3 bg-white border-t border-slate-200 shrink-0 w-full max-w-full overflow-hidden">
                  {/* Phase 2: Selected Image Preview Chip */}
                  {selectedImage && (
                    <div className="flex items-center gap-2 mb-2 p-1.5 bg-slate-100 border border-slate-300 rounded-xl w-fit max-w-full overflow-hidden">
                      <img
                        src={selectedImage.previewUrl}
                        alt="Seçilen belge"
                        className="w-10 h-10 object-cover rounded-lg border border-slate-200 shrink-0"
                      />
                      <div className="text-[11px] truncate max-w-[180px] min-w-0">
                        <span className="font-semibold text-slate-800 block truncate">{selectedImage.fileName}</span>
                        <span className="text-slate-500 text-[10px] block truncate">İrsaliye / Fiş / Taş Fotoğrafı</span>
                      </div>
                      <button
                        type="button"
                        onClick={handleClearSelectedImage}
                        className="p-1 hover:bg-slate-200 rounded-full text-slate-500 hover:text-rose-600 transition-colors ml-1 cursor-pointer shrink-0"
                  ...
                  <div className="flex items-center gap-1 sm:gap-2 bg-slate-50 border border-slate-300 rounded-2xl px-2 sm:px-3 py-1.5 w-full min-w-0 max-w-full overflow-hidden focus-within:border-amber-500 focus-within:ring-2 focus-within:ring-amber-500/20 transition-all">
                    ...
                    <button
                      ...
                      className="p-1.5 shrink-0 text-slate-400 hover:text-amber-600 hover:bg-slate-200/50 rounded-full transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      disabled={isProcessingImage || isLoading}
                      className="p-1.5 shrink-0 text-slate-400 hover:text-blue-600 hover:bg-slate-200/50 rounded-full transition-all cursor-pointer"
                    ...
                    <button
                      ...
                      className={`p-1.5 shrink-0 rounded-full transition-all cursor-pointer ${
                        isListening
                    ...
                    <input
                      ...
                      placeholder={
                        selectedImage
                          ...
                          : isListening
                          ? 'Dinleniyor...'
                          : 'Soru sorun veya irsaliye yükleyin...'
                      }
                      className="flex-1 min-w-0 w-full bg-transparent text-sm text-slate-800 placeholder-slate-400 focus:outline-none"
                    />
                    ...
                    <button
                      ...
                      className="p-1.5 shrink-0 bg-amber-500 hover:bg-amber-600 disabled:bg-slate-200 text-white disabled:text-slate-400 rounded-xl transition-all shadow-sm active:scale-95 disabled:active:scale-100 cursor-pointer"
                    >
